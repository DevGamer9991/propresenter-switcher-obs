...
MainWidget::MainWidget(QWidget *parent) : QDockWidget("Propresenter Controller", parent) {
    ...
    obs_data_t *settings = obs_data_create_from_json_file_safe(profilePath.c_str(), "bak");
    if (settings) {
    ...
    QObject::connect(ipInput, SIGNAL(editingFinished()), SLOT(ipInputEdited()));
...
void MainWidget::ipInputEdited() {
    ...
    obs_data_set_string(settings, "ip", ipInput->text().toStdString().c_str());
    // now save it to json
